import { MapPin, Bus, Radar, Bell } from "lucide-react";
...
function Card({ step, index, total }) {
    ...
    return (<div ref={containerRef} className="relative h-[100vh] w-full">
      <motion.div style={{
            scale,
            opacity,
            y,
...
export function HowItWorks() {
    return (<section id="how-it-works" className="relative">
      ...
      <div className="relative mx-auto mt-16 max-w-7xl px-5">
        <div className="flex flex-col">
          {steps.map((step, index) => (<Card key={index} step={step} index={index} total={steps.length}/>))}
        </div>
      </div>
