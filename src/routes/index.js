...
function HeroAnimation() {
    return (<div className="absolute inset-x-0 -bottom-8 h-48 pointer-events-none">
      <svg aria-hidden viewBox="0 0 800 200" className="h-full w-full opacity-40" preserveAspectRatio="none">
        <path id="heroRoute" d="M 50 150 C 200 150, 400 50, 750 50" fill="none" stroke="var(--color-primary)" strokeWidth="3" strokeDasharray="8 12" className="animate-dash"/>
...
function Index() {
    ...
    return (<main className="relative min-h-screen overflow-hidden bg-[linear-gradient(to_bottom,var(--color-sky-top),var(--color-sky-bottom)_62%,var(--color-background))]">
      <Clouds />
      ...
      <header className="relative z-20 mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 py-5 lg:flex lg:justify-between">
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {navLinks.map((l) => (<a key={l.label} href={l.href} onClick={(e) => {
            ...
            }} className={`story-link text-sm font-bold transition-all duration-300 hover:text-accent hover:scale-110 ${activeSection === l.label ? "text-accent border-b-2 border-accent" : "text-ink/80"}`}>
              {l.label}
            </a>))}
        </nav>
        ...
        <div className="hidden items-center gap-4 lg:flex">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-xl">
              {["English", "Bangla", "Arabic"].map((lang) => (<DropdownMenuItem key={lang} onClick={() => setLanguage(lang)} className="rounded-xl font-bold cursor-pointer">
                  {lang}
                </DropdownMenuItem>))}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <div className="flex items-center gap-2">
            <a href="#" className="rounded-full px-5 py-2.5 text-sm font-bold text-ink transition-colors hover:bg-card">
              Login
            </a>
            <a href="#" className="rounded-full bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground shadow-[0_10px_20px_-10px_var(--color-ink)] transition-transform hover:scale-105">
              Sign Up
            </a>
          </div>
        </div>

        <button onClick={() => setOpen((v) => !v)} aria-label="Toggle menu" className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-card text-ink shadow-sm lg:hidden">
          <Menu className="h-5 w-5"/>
        </button>

        {open && (<div className="col-span-2 rounded-3xl bg-card p-4 shadow-lg animate-scale-in lg:hidden">
            <nav className="flex flex-col gap-1">
              {navLinks.map((l) => (<a key={l.label} href={l.href} onClick={(e) => {
                ...
                }} className={`rounded-xl px-3 py-2 text-sm font-bold hover:bg-secondary ${activeSection === l.label ? "text-primary bg-secondary/30" : "text-ink"}`}>
                  {l.label}
                </a>))}
            </nav>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 pb-24 pt-6 lg:pt-10">
        <div className="relative z-20 mx-auto max-w-2xl text-center">
          ...
          <div className="mb-6 flex justify-center opacity-0 animate-[rise_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards] [animation-delay:400ms]">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white p-2 shadow-2xl animate-float lg:h-20 lg:w-20" role="img" aria-label="PUB Bus Track logo">
        ...
        <div className="relative mt-12 [perspective:1000px] lg:mt-0 lg:flex lg:items-center lg:justify-between">
          ...
          <div className="relative z-10 w-full opacity-0 animate-[slide-in-3d-left_1.2s_cubic-bezier(0.16,1,0.3,1)_forwards] lg:w-[45%]">
            <div className="relative transition-transform duration-700 animate-[float-subtle_5s_ease-in-out_infinite]" style={{
            transformStyle: 'preserve-3d',
            '--resting-rotate': '15deg'
        }}>
              <img src={pundraUni.url} alt="Pundra University main campus building with modern architectural design" className="aspect-[4/3] w-full object-contain [filter:drop-shadow(0_20px_40px_rgba(0,0,0,0.1))]" fetchPriority="high" sizes="(max-width: 1024px) 100vw, 45vw"/>
            </div>
          ...
          <div className="relative z-10 mt-12 w-full opacity-0 animate-[slide-in-3d-right_1.2s_cubic-bezier(0.16,1,0.3,1)_forwards] [animation-delay:300ms] lg:mt-0 lg:w-[45%]">
            <div className="relative transition-transform duration-700 animate-[float-subtle_6s_ease-in-out_infinite]" style={{
            transformStyle: 'preserve-3d',
            '--resting-rotate': '-15deg'
        }}>
              <img src={pubBus.url} alt="PUB Bus Track university shuttle bus with brand livery" className="aspect-[4/3] w-full object-contain [filter:drop-shadow(0_20px_40px_rgba(0,0,0,0.1))]" fetchPriority="high" sizes="(max-width: 1024px) 100vw, 45vw"/>
            </div>
...
\`\`\``}
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 pb-20">
        {/* Download section */}
        <div id="download" className="mx-auto mt-16 max-w-3xl animate-rise rounded-[2rem] bg-card p-7 text-center shadow-[0_28px_50px_-30px_var(--color-ink)] sm:p-10">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <a href="#" className="flex w-full items-center justify-center gap-3 rounded-2xl bg-ink px-6 py-3 text-left text-background transition-transform hover:scale-105 sm:w-auto">
        ...
        ].map((f) => (<div key={f.label} className="rounded-2xl bg-secondary px-4 py-4">
